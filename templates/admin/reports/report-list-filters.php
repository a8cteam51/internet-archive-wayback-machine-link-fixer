...
<div id="wlf-report-filters">
	<form method="get" action="<?php echo esc_url( admin_url() ); ?>">
		...
		<label><?php esc_html_e( 'Created by: ', 'wpcomsp_wayback_link_fixer' ); ?>
			<select class="filter-multiple" name="<?php echo esc_attr( Report_List_View::PARAM_USER_ID ); ?>">
				...
				<?php foreach ( get_users( array( 'role__in' => array( 'administrator', 'editor', 'author' ) ) ) as $wlf_user ) : ?>
					<option value="<?php echo esc_attr( $wlf_user->ID ); ?>" <?php selected( $filters['user_id'], $wlf_user->ID ); ?>>
		...
		<?php if ( is_multisite() ) : ?>
			<label><?php esc_html_e( 'Blog: ', 'wpcomsp_wayback_link_fixer' ); ?>
				<select class="filter-multiple" name="<?php echo esc_attr( Report_List_View::PARAM_BLOG_ID ); ?>">
					<option value=""><?php esc_html_e( 'Any', 'wpcomsp_wayback_link_fixer' ); ?></option>
		...
		<!-- Submit -->
		<input type="submit" value="<?php esc_attr_e( 'Filter', 'wpcomsp_wayback_link_fixer' ); ?>" />
